...
void pwm_pb0_tim3_init(uint32_t pwm_hz, uint32_t duty_permille)
{
    ...
    // 3) TIM3 base
    TIM3->CR1 = 0;	//Borrar todo del timer 3
    ...
    TIM3->CCR3 = (uint16_t)ccr;	//Cuanto tiempo va a estar en alto

    // 4) PWM mode 1 en CH3 + preload
    // CH3 vive en CCMR2
    TIM3->CCMR2 &= ~(TIM_CCMR2_CC3S_Msk | TIM_CCMR2_OC3M_Msk);
    TIM3->CCMR2 |=  (6U << TIM_CCMR2_OC3M_Pos) | TIM_CCMR2_OC3PE;	//Se asegura que el periodo sea el correcto
